...
class LNCTAttendance:
        ...
        try:
            ...
            if not username_field:
                for inp in all_inputs:
                    ...
                    if any(keyword in inp_id for keyword in ["user", "login", "student"]) or any(
                        keyword in inp_name for keyword in ["user", "login", "student"]
                    ):
            ...
            if not password_field:
                for inp in all_inputs:
                    inp_id = inp.get("id", "").lower()
                    inp_name = inp.get("name", "").lower()
                    if any(keyword in inp_id for keyword in ["pass", "pwd"]) or any(
                        keyword in inp_name for keyword in ["pass", "pwd"]
                    ):
            ...
            if not username_field or not password_field:
                ...
                return (
                    False,
                    f"Could not find login fields. Available field names: {available_fields}. Username field: {username_field}, Password field: {password_field}",
                )
            ...
            response = self.session.post(
                self.login_url, data=form_data, timeout=self.timeout, allow_redirects=True
            )
            ...
            if "parentdesk" in response.url.lower() or "dashboard" in response.url.lower():
                return (
                    True,
                    f"Login successful - Redirected to dashboard. Found indicators: {success_found}",
                )
            ...
            return (
                False,
                f"Login status unclear. URL: {response.url}, Page title: {soup.find('title').get_text() if soup.find('title') else 'No title'}",
            )
    ...
    def get_attendance(self):
        ...
        try:
            ...
            attendance_data = {
                "total_classes": self._extract_value(
                    soup, "ctl00_ContentPlaceHolder1_lbltotperiod111", int
                ),
                ...
                "not_applicable": self._extract_value(
                    soup, "ctl00_ContentPlaceHolder1_lbltotaln11", int
                ),
            ...
            attendance_data.update(
                {
                    ...
                    "overall_percentage": attendance_data[
                        "percentage"
            ...
            for table in tables:
                ...
                for row in rows[1:]:  # Skip header
                    ...
                    if len(cells) >= 3:
                        ...
                        if (
                            not subject_name
                            or subject_name.isdigit()
                            or subject_name.lower() in skip_patterns
                            or len(subject_name) < 3
                        ):
                            continue
...
def check_site():
    ...
    try:
        ...
        return jsonify({"success": False, "error": str(e), "site_accessible": False})
...
def get_attendance():
    try:
        if request.method == "POST":
            ...
            if not data:
                return (
                    jsonify({"success": False, "error": "JSON data required for POST requests"}),
                    400,
                )
        ...
        if not login_success:
            return (
                jsonify(
                    {"success": False, "error": "Authentication failed", "details": login_message}
                ),
                401,
            )
...
def test_login():
    try:
        ...
        if login_success:
            ...
            return jsonify(
                {"success": True, "login_status": login_message, "dashboard_access": dashboard_msg}
            )
        else:
            return (
                jsonify({"success": False, "error": "Login failed", "details": login_message}),
                401,
            )
        ...
        return jsonify(
            {
                ...
                "login_attempt": {"success": login_success, "message": login_message},
            }
        )
