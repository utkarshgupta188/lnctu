    ...
    def login(self, username, password):
        try:
            ...
            username_field = next(
                (
                    f
                    for f in ["txtUserName", "txtUsername", "UserId", "txtUserId"]
            ...
            password_field = next(
                (
                    f
                    for f in ["txtPassword", "Password", "txtPass"]
                    if soup.find("input", {"name": f})
                ),
                None,
            )
            ...
            submit_btn = soup.find("input", {"type": "submit"}) or soup.find(
                "input", {"type": "button"}
            )
            ...
            res = self.session.post(
                self.login_url, data=form_data, timeout=15, allow_redirects=True
            )
...
def attendance(username: str = "", password: str = ""):
    ...
    user_sessions[username] = {"lnct": lnct, "last_login": datetime.now()}
