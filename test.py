...
class LNCTAttendance:
    ...
    def login(self, username, password):
        try:
            ...
            username_field = next((f for f in ['txtUserName', 'txtUsername', 'username', 'UserId', 'txtUserId'] if soup.find('input', {'name': f})), None)
            password_field = next((f for f in ['txtPassword', 'password', 'Password', 'txtPass'] if soup.find('input', {'name': f})), None)
...
def check_site():
    try:
        ...
        return jsonify({'success': True, 'status': res.status_code, 'accessible': res.status_code == 200})
    except Exception as e:
