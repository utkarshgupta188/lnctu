...
# Cache session for 1 minute per user
...
class LNCTAttendance:
    ...
    def login(self, username, password):
        try:
            ...
            username_field = next(
                (
                    f
                    for f in ["txtUserName", "txtUsername", "username", "UserId", "txtUserId"]
            ...
            password_field = next(
                (
                    f
                    for f in ["txtPassword", "password", "Password", "txtPass"]
                    if soup.find("input", {"name": f})
                ),
                None,
            )
    ...
    def get_attendance(self):
        try:
            ...
            data = {
                "total_classes": self.extract_value(
                ...
                "not_applicable": self.extract_value(
            ...
            data["percentage"] = (
                round((data["present"] / data["total_classes"]) * 100, 2)
...
def get_cached_session(username, password):
...
def check_site():
    try:
        lnct = LNCTAttendance()
        res = lnct.session.get(lnct.login_url, timeout=5)
        return jsonify(
            {"success": True, "status": res.status_code, "accessible": res.status_code == 200}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e), "accessible": False})


if __name__ == "__main__":
    print("🚀 LNCT Attendance API Running on http://localhost:5000")
